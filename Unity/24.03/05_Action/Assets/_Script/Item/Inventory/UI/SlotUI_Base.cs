...
public class SlotUI_Base : MonoBehaviour
{
    ...
    /// </summary>
    /// <param name="slot">연결할 슬롯</param>
    public virtual void InitializeSlot(InvenSlot slot)
    {
        invenSlot = slot;
        invenSlot.onSlotItemChange = Refresh;   // 슬롯의 아이템에 변경이 있을때 Refresh함수 실행(이전에 연결된 함수는 무시)
        Refresh();                              // 첫 화면 갱신
    }
    ...
    private void Refresh()
    {
        ...
        {
            itemIcon.sprite = InvenSlot.ItemData.itemIcon;  // 스프라이트 이미지설정

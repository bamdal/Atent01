...
{
    readonly Vector2Int[] dirs = { new(1, 0), new(0, 1), new(-1, 0), new(0, -1) };
    protected override void OnSpecificAlgorithmExcute()
    {
        ...
        // 필드의 한곳을 랜덤선택
        int firstIndex = notInMaze[0];                          // 미로에 포함되지 않은 셀 중에서 하난 꺼내기
        notInMaze.RemoveAt(0);
        WilsonCell first = cells[firstIndex] as WilsonCell;
        first.isMazeMember = true;                              // 꺼낸 셀 미로에 포함시키기


        while(notInMaze.Count > 0)  // notInMaze에 포함된 아이템의 개수가 0보다 크다 == 아직 미로에 포함되지 않은 셀이남아있다
        {
            // 미로에 포함도지 않은 셀 중 하나를 랜덤선택
            ...
            } while (!current.isMazeMember);    // 미로에 포함된 셀에 도착할때까지 반복

            // 셀의 위치에서 미로에 포함된 영역에 도착할 때까지의 경로를 미로에 포함시킨다
            ...
            while (path != current) // 시작부터 current에 도착할 때까지 돌기
            {
                ...
                ConnectPath(path, path.next);                   // 다음 셀과 길을 연결하고
                path = path.next;                               // 다음셀로 넘어가기
            }   // 모든셀이 포함될때까지 반복
        }
    ...
    WilsonCell GetNeighbor(WilsonCell cell)
    {
        ...
        return cells[GridToIndex(neighborPos)] as WilsonCell;
    }
}

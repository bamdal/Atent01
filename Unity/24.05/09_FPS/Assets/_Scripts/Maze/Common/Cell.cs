...
{
    /// <summary>
    /// 이 셀에 연결된 길을 기록하는 변수(북동남서 순서대로 해당하는 비트에 1이 세팅되어 있으면 길이 있다(벽이 없다). 0 이 세팅되어 있으면 길이 없다(벽이 있다)
    /// </summary>
    byte path;
    ...
    /// 생성자
    /// </summary>
    /// <param name="x">셀의 x위치</param>
    /// <param name="y">셀의 y위치</param>
    ...
    public bool CornerPathCheck(Direction dir1, Direction dir2)
    {
        ...
        if(corner == (Direction.North | Direction.East)
            ...
            || corner == (Direction.East | Direction.South))
        {

        }

        return IsPath(dir1) && IsPath(dir2);
    }
}

...
public class AsyncLoadingScene : MonoBehaviour
{
    ...
    IEnumerator AsyncLoadScene()
    {
        ...
        async.allowSceneActivation = false;                 // 자동으로 씬 전환되지 않도록 하기

        while (loadRatio < 1.0f) 
        {
            loadRatio = async.progress + 0.1f;  // 로딩 지행율에 따라 loadRatio 설정
